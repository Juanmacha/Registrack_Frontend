...
const getEstadoBadge = (estado) => {
  ...
  if (estadoLower.includes("revisión") || estadoLower.includes("activo")) {
    return <span className="px-3 py-1 text-green-700 bg-green-100 rounded-full text-xs font-semibold">Active</span>;
  }
  if (estadoLower.includes("pendiente")) {
    return <span className="px-3 py-1 text-yellow-800 bg-yellow-100 rounded-full text-xs font-semibold">Pending</span>;
  }
  return <span className="px-3 py-1 text-red-700 bg-red-100 rounded-full text-xs font-semibold">Cancel</span>;
};
        ...
        <table className="table-auto w-full divide-y divide-gray-100">
          <thead className="text-left text-sm text-gray-500 bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 font-medium text-center">Persona</th>
          ...
          <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
            {datosMock.map((item) => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 text-center">{item.tipoPersona}</td>
